...
### Step 5: Write to GBrain

If gbrain is available, persist the context summary as a brain page for cross-session retrieval:
...
Use the same summary content from Step 2 and the YAML frontmatter from Step 4. Include branch, timestamp, files modified, decisions, and remaining work. This makes the context searchable via `gbrain search` in future sessions. If gbrain is not available, skip this step silently.
