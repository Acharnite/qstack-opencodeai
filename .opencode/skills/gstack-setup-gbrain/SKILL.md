...
>   ✅ Zero local state — only `~/.config/opencode/opencode.jsonc` MCP registration
...
resting state in `~/.config/opencode/opencode.jsonc` mode 0600.
...
~10ms. The token's resting state is `~/.config/opencode/opencode.jsonc` (mode 0600 — Claude
...
- Token: stored in ~/.config/opencode/opencode.jsonc (do not commit; never written to AGENTS.md)
...
in to git in many projects). It lives only in `~/.config/opencode/opencode.jsonc` where
